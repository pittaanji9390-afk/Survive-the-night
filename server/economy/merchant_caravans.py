...
Module: merchant_caravans
Description: Dynamic NPC Trade Caravans, Supply-Demand Routing & Tariff Policies
...
@dataclass
class MerchantCaravansStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
    payload: Dict[str, Any] = field(default_factory=dict)
    checksum: str = ""

class MerchantCaravans:
    """
    Production-grade implementation of Dynamic NPC Trade Caravans, Supply-Demand Routing & Tariff Policies.
    """
    def __init__(self, config: Optional[MerchantCaravansConfig] = None) -> None:
        self.config = config or MerchantCaravansConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[MerchantCaravansStateSnapshot] = []
        self._subscribers: List[Callable[[MerchantCaravansStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> MerchantCaravansStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = MerchantCaravansStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[MerchantCaravansStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[MerchantCaravansStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[MerchantCaravansConfig] = None) -> MerchantCaravans:
    return MerchantCaravans(config)
