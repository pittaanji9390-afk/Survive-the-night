...
Module: heat_map_generator
Description: Player Death & Resource Extraction 2D Spatial Density Heatmaps
...
@dataclass
class HeatMapGeneratorStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class HeatMapGenerator:
    """
    Production-grade implementation of Player Death & Resource Extraction 2D Spatial Density Heatmaps.
    """
    def __init__(self, config: Optional[HeatMapGeneratorConfig] = None) -> None:
        self.config = config or HeatMapGeneratorConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[HeatMapGeneratorStateSnapshot] = []
        self._subscribers: List[Callable[[HeatMapGeneratorStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> HeatMapGeneratorStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = HeatMapGeneratorStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[HeatMapGeneratorStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[HeatMapGeneratorStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[HeatMapGeneratorConfig] = None) -> HeatMapGenerator:
    return HeatMapGenerator(config)
