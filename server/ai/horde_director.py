...
Module: horde_director
Description: Dynamic Wave Pacing, Threat Budget Allotment & Blood Moon Orchestrator
...
@dataclass
class HordeDirectorStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class HordeDirector:
    """
    Production-grade implementation of Dynamic Wave Pacing, Threat Budget Allotment & Blood Moon Orchestrator.
    """
    def __init__(self, config: Optional[HordeDirectorConfig] = None) -> None:
        self.config = config or HordeDirectorConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[HordeDirectorStateSnapshot] = []
        self._subscribers: List[Callable[[HordeDirectorStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> HordeDirectorStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = HordeDirectorStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[HordeDirectorStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[HordeDirectorStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[HordeDirectorConfig] = None) -> HordeDirector:
    return HordeDirector(config)
