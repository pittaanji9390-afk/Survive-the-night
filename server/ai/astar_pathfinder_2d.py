...
Module: astar_pathfinder_2d
Description: Grid-Based A* Pathfinding with Dynamic Obstacle Cost Weights
...
@dataclass
class AstarPathfinder2dStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class AstarPathfinder2d:
    """
    Production-grade implementation of Grid-Based A* Pathfinding with Dynamic Obstacle Cost Weights.
    """
    def __init__(self, config: Optional[AstarPathfinder2dConfig] = None) -> None:
        self.config = config or AstarPathfinder2dConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[AstarPathfinder2dStateSnapshot] = []
        self._subscribers: List[Callable[[AstarPathfinder2dStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> AstarPathfinder2dStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = AstarPathfinder2dStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[AstarPathfinder2dStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[AstarPathfinder2dStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[AstarPathfinder2dConfig] = None) -> AstarPathfinder2d:
    return AstarPathfinder2d(config)
