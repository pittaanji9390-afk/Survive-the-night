...
Module: ecs_registry
Description: High-Performance Sparse Set Entity Component System (ECS)
...
@dataclass
class EcsRegistryStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class EcsRegistry:
    """
    Production-grade implementation of High-Performance Sparse Set Entity Component System (ECS).
    """
    def __init__(self, config: Optional[EcsRegistryConfig] = None) -> None:
        self.config = config or EcsRegistryConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[EcsRegistryStateSnapshot] = []
        self._subscribers: List[Callable[[EcsRegistryStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> EcsRegistryStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = EcsRegistryStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[EcsRegistryStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[EcsRegistryStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[EcsRegistryConfig] = None) -> EcsRegistry:
    return EcsRegistry(config)
