...
Module: fire_propagation
Description: Cellular Automata Combustion Thermodynamics & Heat Conduction Grid
...
@dataclass
class FirePropagationStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class FirePropagation:
    """
    Production-grade implementation of Cellular Automata Combustion Thermodynamics & Heat Conduction Grid.
    """
    def __init__(self, config: Optional[FirePropagationConfig] = None) -> None:
        self.config = config or FirePropagationConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[FirePropagationStateSnapshot] = []
        self._subscribers: List[Callable[[FirePropagationStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> FirePropagationStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = FirePropagationStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[FirePropagationStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[FirePropagationStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[FirePropagationConfig] = None) -> FirePropagation:
    return FirePropagation(config)
