...
Module: bsp_labyrinth
Description: Binary Space Partitioning Room Splitter, Puzzle Lock Engine & Corridors
...
@dataclass
class BspLabyrinthStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class BspLabyrinth:
    """
    Production-grade implementation of Binary Space Partitioning Room Splitter, Puzzle Lock Engine & Corridors.
    """
    def __init__(self, config: Optional[BspLabyrinthConfig] = None) -> None:
        self.config = config or BspLabyrinthConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[BspLabyrinthStateSnapshot] = []
        self._subscribers: List[Callable[[BspLabyrinthStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> BspLabyrinthStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = BspLabyrinthStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[BspLabyrinthStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[BspLabyrinthStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[BspLabyrinthConfig] = None) -> BspLabyrinth:
    return BspLabyrinth(config)
