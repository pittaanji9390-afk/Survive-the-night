...
Module: cellular_dungeon
Description: Multi-Pass Cellular Automata Cave Carver with BFS Connectivity Assurance
...
@dataclass
class CellularDungeonStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class CellularDungeon:
    """
    Production-grade implementation of Multi-Pass Cellular Automata Cave Carver with BFS Connectivity Assurance.
    """
    def __init__(self, config: Optional[CellularDungeonConfig] = None) -> None:
        self.config = config or CellularDungeonConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[CellularDungeonStateSnapshot] = []
        self._subscribers: List[Callable[[CellularDungeonStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> CellularDungeonStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = CellularDungeonStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[CellularDungeonStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[CellularDungeonStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[CellularDungeonConfig] = None) -> CellularDungeon:
    return CellularDungeon(config)
