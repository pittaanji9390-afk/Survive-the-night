...
Module: boss_ai_state_machine
Description: Multi-Phase Boss Metamorphosis, Enrage Triggers & Telegraphed Attacks
...
@dataclass
class BossAiStateMachineConfig:
    enabled: bool = True
...
@dataclass
class BossAiStateMachineStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class BossAiStateMachine:
    """
    Production-grade implementation of Multi-Phase Boss Metamorphosis, Enrage Triggers & Telegraphed Attacks.
    """
    def __init__(self, config: Optional[BossAiStateMachineConfig] = None) -> None:
        self.config = config or BossAiStateMachineConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[BossAiStateMachineStateSnapshot] = []
        self._subscribers: List[Callable[[BossAiStateMachineStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> BossAiStateMachineStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = BossAiStateMachineStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[BossAiStateMachineStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[BossAiStateMachineStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[BossAiStateMachineConfig] = None) -> BossAiStateMachine:
    return BossAiStateMachine(config)
