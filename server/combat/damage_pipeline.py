...
Module: damage_pipeline
Description: Multi-Stage Combat Mitigation, Armor Scaling & Critical Hit Pipeline
...
@dataclass
class DamagePipelineStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class DamagePipeline:
    """
    Production-grade implementation of Multi-Stage Combat Mitigation, Armor Scaling & Critical Hit Pipeline.
    """
    def __init__(self, config: Optional[DamagePipelineConfig] = None) -> None:
        self.config = config or DamagePipelineConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[DamagePipelineStateSnapshot] = []
        self._subscribers: List[Callable[[DamagePipelineStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> DamagePipelineStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = DamagePipelineStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[DamagePipelineStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[DamagePipelineStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[DamagePipelineConfig] = None) -> DamagePipeline:
    return DamagePipeline(config)
