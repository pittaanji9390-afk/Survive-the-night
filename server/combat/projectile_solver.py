...
Module: projectile_solver
Description: Ballistic Arc Trajectory, Drag Coefficient & Raycast Intersections
...
@dataclass
class ProjectileSolverStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class ProjectileSolver:
    """
    Production-grade implementation of Ballistic Arc Trajectory, Drag Coefficient & Raycast Intersections.
    """
    def __init__(self, config: Optional[ProjectileSolverConfig] = None) -> None:
        self.config = config or ProjectileSolverConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[ProjectileSolverStateSnapshot] = []
        self._subscribers: List[Callable[[ProjectileSolverStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> ProjectileSolverStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = ProjectileSolverStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[ProjectileSolverStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[ProjectileSolverStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[ProjectileSolverConfig] = None) -> ProjectileSolver:
    return ProjectileSolver(config)
