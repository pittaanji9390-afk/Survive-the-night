...
Description: Buff & Debuff Lifecycle, Damage Over Time Ticks & Synergy Interactions
...
@dataclass
class StatusEffectsStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class StatusEffects:
    """
    Production-grade implementation of Buff & Debuff Lifecycle, Damage Over Time Ticks & Synergy Interactions.
    """
    def __init__(self, config: Optional[StatusEffectsConfig] = None) -> None:
        self.config = config or StatusEffectsConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[StatusEffectsStateSnapshot] = []
        self._subscribers: List[Callable[[StatusEffectsStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> StatusEffectsStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = StatusEffectsStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[StatusEffectsStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[StatusEffectsStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[StatusEffectsConfig] = None) -> StatusEffects:
    return StatusEffects(config)
