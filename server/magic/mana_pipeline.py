...
Module: mana_pipeline
Description: Mana Channeling, Elemental Affinity Modifiers & Spell Overcharge
...
@dataclass
class ManaPipelineStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class ManaPipeline:
    """
    Production-grade implementation of Mana Channeling, Elemental Affinity Modifiers & Spell Overcharge.
    """
    def __init__(self, config: Optional[ManaPipelineConfig] = None) -> None:
        self.config = config or ManaPipelineConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[ManaPipelineStateSnapshot] = []
        self._subscribers: List[Callable[[ManaPipelineStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> ManaPipelineStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = ManaPipelineStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[ManaPipelineStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[ManaPipelineStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[ManaPipelineConfig] = None) -> ManaPipeline:
    return ManaPipeline(config)
