...
Module: colonist_needs
Description: Deep Physiological Need Curves (Hunger, Fatigue, Morale, Comfort, Social)
...
@dataclass
class ColonistNeedsStateSnapshot:
    timestamp: float = 0.0
    sequence_id: int = 0
...
class ColonistNeeds:
    """
    Production-grade implementation of Deep Physiological Need Curves (Hunger, Fatigue, Morale, Comfort, Social).
    """
    def __init__(self, config: Optional[ColonistNeedsConfig] = None) -> None:
        self.config = config or ColonistNeedsConfig()
        self._is_running = False
        self._tick_counter = 0
        self._last_tick_time = time.time()
        self._history_log: List[ColonistNeedsStateSnapshot] = []
        self._subscribers: List[Callable[[ColonistNeedsStateSnapshot], None]] = []
    ...
    def tick(self, delta_time: float) -> ColonistNeedsStateSnapshot:
        """Main tick synchronization dispatch."""
        self._tick_counter += 1
        snapshot = ColonistNeedsStateSnapshot(
            timestamp=time.time(),
    ...
    def subscribe(self, callback: Callable[[ColonistNeedsStateSnapshot], None]) -> None:
        if callback not in self._subscribers:
            self._subscribers.append(callback)

    def unsubscribe(self, callback: Callable[[ColonistNeedsStateSnapshot], None]) -> None:
...
def create_instance(config: Optional[ColonistNeedsConfig] = None) -> ColonistNeeds:
    return ColonistNeeds(config)
